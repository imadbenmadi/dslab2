...
\begin{abstract}
\noindent
This paper presents the complete mathematical methodology of PCNME, the
Predictive Cloud-Native Mobile Edge framework for intelligent task offloading
in three-tier IoT--Fog--Cloud systems. The problem of routing computation-intensive
tasks from resource-constrained IoT vehicles to fog or cloud servers is formulated
as a multi-objective combinatorial optimization problem with deadline and capacity
constraints. We define all system variables and derive formal expressions for
\textit{\textbf{execution cost, task latency, and energy consumption}}. The methodology integrates
five algorithmic components: an Execution Cost classifier that pre-filters tasks
before optimization, an MMDE-enhanced NSGA-II multi-objective optimizer that
produces Pareto-optimal offline routing plans, a behavioral cloning pipeline
that seeds a Deep Q-Network agent from those plans, an online DQN agent that
adapts placement decisions to live network conditions, and a GPS-driven proactive
handoff predictor that prevents task failures when vehicles cross fog coverage
boundaries. Every variable, equation, and parameter is defined formally and its
physical interpretation is stated. The objective of this methodology section is
to provide a complete, self-contained mathematical foundation from which an
implementation can be derived without ambiguity.
\end{abstract}
...
\end{equation}
where $\bm{p}_k \in \mathbb{R}^2$ is its two-dimensional position in metres,
...
source step to the sink step that maximises total execution time. End-to-end
...
The state vector $\mathbf{s} \in \mathbb{R}^{13}$ observed by the agent
at decision time $t$ for step $g_j$ submitted by vehicle $v_i$ is:
\begin{equation}
  \mathbf{s} = \bigl(
    \rho_1(t),\; \rho_2(t),\; \rho_3(t),\; \rho_4(t),\;
    q_1(t),\; q_2(t),\; q_3(t),\; q_4(t),\;
    \widehat{\mathrm{EC}}_j,\; \widehat{B}(t),\;
    \widehat{s}_i(t),\; \widehat{T}_{\text{exit}},\;
    \widehat{d}_j^{\text{rem}}
  \bigr)
  \label{eq:state}
\end{equation}
...
\begin{itemize}[leftmargin=2em]
  \item $\rho_k(t) \in [0,1]$: CPU utilisation of fog node $f_k$ (four fog nodes)
  \item $q_k(t) / q_{\max}$: queue depth of fog node $f_k$ normalised by maximum
  \item $\widehat{\mathrm{EC}}_j = \min(\mathrm{EC}(g_j) / \theta, 1)$: normalised EC value
  \item $\widehat{B}(t) = B_{\text{used}}(t) / B_{\max}$: upload link utilisation
  \item $\widehat{s}_i(t) = s_i(t) / s_{\max}$: vehicle speed normalised
  \item $\widehat{T}_{\text{exit}} = \min(T_{\text{exit}} / T_{\max}, 1)$: normalised zone exit time
  \item $\widehat{d}_j^{\text{rem}} = d_j^{\text{local}} / D_\tau$: remaining deadline fraction
\end{itemize}
...
With $|\mathbf{s}| = 13$, $h_1 = 256$, $h_2 = 128$, $|\mathcal{A}| = 5$:
\begin{equation}
  \mathcal{O}_{\text{infer}} = O(13 \cdot 256 + 256 \cdot 128 + 128 \cdot 5)
  = O(36,\!288)
