            ...
            <header className="bg-black bg-opacity-50 backdrop-blur border-b border-gray-700">
                <div className="max-w-7xl mx-auto px-6 py-8">
                    ...
                    <p className="text-gray-400">
                        Real-Time Dashboard • Istanbul • 50 Vehicles • 4 Fog Nodes
                    </p>
                    <div className="mt-4">
                        <a
                            href={mapOnly ? '/' : '/map'}
                        ...
                        >
                            {mapOnly ? 'Back to Main Dashboard' : 'Open Full Istanbul Live Map'}
                        </a>
                        {!mapOnly && (
                            <a
                                href="/agents"
                ...
                <div className="max-w-7xl mx-auto px-6 py-6">
                    <div className="flex flex-wrap gap-4 items-center">
                        ...
                        <div className="flex gap-2">
                            {['baseline1', 'baseline2', 'baseline3', 'proposed'].map((type) => (
                                <button
                                    key={type}
                                    ...
                                    disabled={isRunning}
                                >
                                    {type === 'baseline1' && 'Baseline 1 (NSGA-II)'}
                                    {type === 'baseline2' && 'Baseline 2 (TOF)'}
                                    {type === 'baseline3' && 'Baseline 3 (MMDE)'}
                        ...
                        <div className="flex gap-2 ml-auto">
                            ...
                            <button
                                onClick={() => useSystemStore.getState().retrainAgents()}
                    ...
                    <div className="mt-6">
                        <div className="flex justify-between mb-2">
                            ...
                            <span className="text-sm font-mono">
                                {progress.toFixed(1)}% • {metrics?.simulationTime?.toFixed(1)}s /
                                900s
                            </span>
                        </div>
...
const MetricCard = ({ title, value, unit, target, color, inverse }) => {
    ...
    const isTarget = target && !inverse ? value >= target : inverse ? value <= target : false;
    ...
    return (
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            ...
            {target && (
                <p className={`text-xs mt-2 ${isTarget ? 'text-green-400' : 'text-red-400'}`}>
                    Target: {target}
                    {unit} {isTarget ? '[OK] ' : '✗'}
                </p>
            )}
    ...
    return (
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
                <div>
                    <div className="flex justify-between mb-2">
                        <span className="text-sm text-gray-400">Bandwidth Used</span>
                        <span className="text-sm font-mono">
                            {metrics.network?.bandwidthUsed?.toFixed(1)}Mbps / 100Mbps
                        </span>
                    </div>
                    <div className="w-full bg-gray-700 rounded-full h-2">
                        <div
    ...
    return (
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-6">
                <div>
                    <p className="text-sm text-gray-400 mb-3">Agent 1 - Task Placement</p>
                    <p className="text-2xl font-bold text-green-400">
                        {metrics.agents?.agent1Latency?.toFixed(2)}ms
                    </p>
                    <p className="text-xs text-gray-500 mt-1">Decision latency</p>
                </div>
                <div>
                    <p className="text-sm text-gray-400 mb-3">Agent 2 - SDN Routing</p>
        ...
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-6">
                <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">Vehicle transitions tracked</p>
                </div>
                <div>
                    <p className="text-sm text-gray-400 mb-3">Task Migrations</p>
                    <p className="text-2xl font-bold text-purple-400">
                        {metrics.handoff?.taskMigrations || 0}
                    </p>
                    <p className="text-xs text-gray-500 mt-1">Tasks moved during handoff</p>
                </div>
            </div>
        </div>
    );
...
const CityVisualization = ({ metrics, fullScreen = false }) => {
    ...
    const toLatLng = (x, y) => {
        const normX = (x - bounds.xMin) / Math.max(1, bounds.xMax - bounds.xMin);
        const normY = (y - bounds.yMin) / Math.max(1, bounds.yMax - bounds.yMin);
    ...
    const preinstalledLinks = offloads.filter((o) => o?.network?.preinstalled).length;
    const droppedLinks = offloads.filter((o) => o?.network?.packetDrop).length;
    const fogRelayLinks = offloads.filter((o) => o?.leg === 'fog_to_cloud').length;

    return (
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            <h3 className="text-lg font-semibold mb-4">Istanbul Real Road Network</h3>
            <div className="space-y-4">
                <div
                ...
                <div className="text-sm text-gray-400 space-y-1">
                    <p>
                        Fog nodes: {fogNodes.length || 4} | Vehicles: {vehicles.length || 0}
                    </p>
                    <p>
                        Active offloads: {offloads.length || 0} (Fog: {fogOffloads.length || 0},
                        Cloud: {cloudOffloads.length || 0}) | Sim time: {map.simulationTime || 0}s
                    </p>
                    <p>
                        SDN links: preinstalled {preinstalledLinks} | packet drops {droppedLinks} |
                        fog-cloud relays {fogRelayLinks}
                    </p>
                    <p>
                        Mobility: handoffs {metrics?.handoff?.count || 0} | migrations{' '}
                        {metrics?.handoff?.taskMigrations || 0}
                    </p>
                    <p>
                        Open route:{' '}
        ...
        <div className="bg-gray-800 bg-opacity-50 backdrop-blur border border-gray-700 rounded-lg p-6">
            ...
            <div className="space-y-4">
                ...
                <div>
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="text-sm font-medium">
                            {isRetraining ? 'Retraining in progress...' : 'Ready for retraining'}
                        </span>
                    </div>
                    {isRetraining && (
                        <div className="w-full bg-gray-700 rounded-full h-2">
                ...
                <div className="pt-4 border-t border-gray-700 text-xs text-gray-400 space-y-2">
                    <p>[OK] Agent 1: Task Placement DQN</p>
                    <p>[OK] Agent 2: SDN Routing Network</p>
                    <p>[OK] Learning from recent tasks</p>
                </div>
